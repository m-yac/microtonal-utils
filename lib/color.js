...
/**
  * Given a magnitude, an interval made up of only primes > 3, and a degree,
  * returns the corresponding interval in color notation
  *
  ...
  * @returns {Interval}
  */
function colorFromSymb(m, iNo23, d) {
  ...
  return iNo23.mul(Interval([e2,e3]));
}
...
function colorNote(a,b, opts) {
  ...
  for (const [p,e] of iNo23.factors()) {
    const j = colorFromSymb(0, Interval(p), 1).pow(e);
    e2 = e2.sub(j.expOf(2));
    e3 = e3.sub(j.expOf(3));
  }
...
function colorFromNote(iNo23, pyi) {
  let [e2,e3] = [pyi.expOf(2).valueOf(), pyi.expOf(3).valueOf()];
  for (const [p,e] of iNo23.factors()) {
    const j = colorFromSymb(0, Interval(p), 1).pow(e);
    e2 = e2.add(j.expOf(2));
    e3 = e3.add(j.expOf(3));
...
function colorTemperament(a,b) {
  ...
  let curr_diff = colorFromSymb(m, iNo23, d-1).red().distance();
  while (curr_diff.compare(last_diff) < 0) {
    segOffset++;
    last_diff = curr_diff;
    curr_diff = colorFromSymb(m, iNo23, d-segOffset).red().distance();
  }
