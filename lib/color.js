...
function colorPrefix(a,b, opts) {
  ...
  let {verbosity, addCosAfterDeg, hideMagnitude, useFullMagnitude, keepTrailingHyphen, addHTMLExps} = opts || {};
  if (verbosity == undefined) { verbosity = 0; }
  ...
  if (m != 0 && !hideMagnitude) {
    mAbs = Math.abs(m);
    if (verbosity == 0) {
      if (addHTMLExps && mAbs > 2) {
        mStr = (m > 0 ? "L" : "s") + "<sup>" + mAbs + "</sup>";
      }
      else {
