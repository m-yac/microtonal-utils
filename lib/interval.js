...
Interval.prototype = {
   ...
   * each k will either be a positive integer between 2 and k_max, a negative
   * integer between -1 and (1-k_max), or a fraction between 0 and 1 with
   * denominator <= k_max.
   *
  ...
  "invIso1": function(k_max) {
    ...
    for (let k = 2; k <= k_max; k++) {
      // look for an i such that this == i.iso1(1-k)
      try {
        const i = this.iso1(1,1-k);
        if (i.hasFactors()) {
          if (thisIsFrac) { res.push([i, Fraction(1-k)]); }
          else            { return  [[i, Fraction(1-k)]]; }
        }
      } catch (_) { }
      // look for i such that this == i.iso1(n/k) for 0 < n/k < 1
      for (let n = 1; n < k; n++) {
        try {
          const r = Fraction(k,n);
          if (r.d == n) {
            const i = this.iso1(r);
            if (i.hasFactors()) {
              if (thisIsFrac) { res.push([i, r.inverse()]); }
              else            { return  [[i, r.inverse()]]; }
            }
          }
        } catch (_) { }
      }
      // look for an i such that this == i.iso1(k)
      try {
        const i = this.iso1(1,k);
        if (i.hasFactors()) {
          if (thisIsFrac) { res.push([i, Fraction(k)]); }
          else            { return  [[i, Fraction(k)]]; }
        }
      } catch (_) { }
    }
