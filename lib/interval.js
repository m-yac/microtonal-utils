...
Interval.prototype = {
   ...
   */
  "toFracBig": function() {
    let ret = BigFraction(1);
    for (const i in keys(this)) {
      if (this[i].d == 1) {
        // we don't use `pow` from fraction.js because it doesn't support BigInts
        let m = this[i].s * this[i].n;
        if (m < 0) {
          ret = ret.mul(1, BigInt(i) ** (-1n * m));
        } else {
          ret = ret.mul(BigInt(i) ** m);
        }
      } else {
  ...
  "toNthRootBig": function() {
    let n = 1n;
    for (const i in keys(this)) {
      n = BigInt(bigInt.lcm(n, this[i].d));
    }
    return { k: this.pow(n).toFracBig(), n: n };
  },
  ...
  "compare": function(a,b) {
    ...
    // uses the fact that a <= b iff a^(1/n) <= b^(1/n)
    return diff.toNthRootBig().k.compare(1);
  },
