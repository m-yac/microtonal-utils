...
const parse = function(a, b) {
  ...
  // if the input is a monzo
    ...
    let ret = {};
    if (a.length > 0) {
      if (!Fraction(a[0]).equals(0)) {
        ret[2] = Fraction(a[0]);
      }
      let i = 1;
      for (let p = 3; i < a.length; p += 2) {
        if (pf.isPrime(p)) {
          if (!Fraction(a[i]).equals(0)) {
            ret[p] = Fraction(a[i]);
          }
          i++;
        }
...
Interval.prototype = {
  ...
  "compare": function(a,b) {
    ...
    try {
      // do an exact comparison if k is finite and small enough
  ...
  "toMonzo": function() {
    ...
    let [ret, isFrac] = [[], true];
    if (2 <= max_p) {
      ret.push(this[2] || Fraction(0));
      isFrac &= !this[2] || this[2].d == 1;
    }
    for (let p = 3; p <= max_p; p += 2) {
      if (pf.isPrime(p)) {
        ret.push(this[p] || Fraction(0));
