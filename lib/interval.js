...
const {unBigFraction, cachedLog2, keys} = require('./utils.js');
...
Interval.prototype = {
  ...
  "valueOf_log": function(a,b) {
    ...
    // (with special cases for base == 1 and base == 2)
    let [base, base_log2, gp] = [new Interval(2), 1, 2];
    if (a != undefined || b != undefined) {
      base = new Interval(a,b);
      if (base.equals(1)) {
        return Infinity; // k / 0 == Infinity if k > 0
      }
      if (!base.equals(2)) {
        base_log2 = base.valueOf_log();
        gp = Math.max(...Object.keys(base));
      }
    }
    const g = (this[gp] || BigFraction(0)).div(base[gp]);
    let ret = g.valueOf();
    for (const i in keys(this, base)) {
      ...
      ret += e.valueOf() * cachedLog2(i)[0] / base_log2;
    }
    return ret;
  },
