...
function evalExpr(e, r, opts, state) {
  ...
  if (Array.isArray(e)) {
    ...
    if (e[0] == "!refIntvToA4") {
      ...
      const ps = e[3].map(ei => evalExpr(ei, r, opts, state).val);
      const [d, loc] = [e[4], e[5]];
      // ensure the list is decreasing
      if (ps.length > 0 && !ps.map(x => x < 1 ? x.recip().valueOf() : x.valueOf())
                              .every((x,i) => i == 0 || x <= ps[i-1])) {
        throw new Error("Invalid color prefix (non-decreasing)");
      }
      // ensure the list has no cancellation
      if (ps.length > 0 && !ps.every((x,i) => i == 0 || !x.mul(ps[i-1]).equals(1))) {
        throw new Error("Invalid color prefix (cancellation)");
      }
      const i = ps.reduce(((a,b) => a.mul(b)), Interval(1));
      return { val: colorFromSymb(cs, m, i, d) };
    }
    else if (e[0] == "!clrNote") {
      const ps = e[1].map(ei => evalExpr(ei, r, opts, state).val);
      ...
      const [p, e, loc] = [e[1], e[2], e[3]];
      if (p == 1) { return { val: Interval(11).pow(e) }; }
      if (p == 3) { return { val: Interval(13).pow(e) }; }
      if (!pf.isPrime(p)) {
        throw new OtherError("Expected a prime number", loc);
      }
      return { val: Interval(p).pow(e) };
    }
    else if (e[0] == "!clrMPs") {
      const [ps, loc] = [e[1].map(ei => evalExpr(ei, r, opts, state).val), e[2]];
      // ensure the list is decreasing
      if (ps.length > 0 && !ps.every((x,i) => i == 0 || x <= ps[i-1])) {
        throw new Error("Invalid color multi prefix (non-decreasing)");
      }
